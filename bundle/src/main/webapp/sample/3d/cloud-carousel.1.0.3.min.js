...
(function($) {
	function Reflection(a, b, c) {
		...
		parent = $(a.parentNode);
		if ($.browser.msie) {
			this.element = d = parent.append(
					"<img class='reflection' style='position:absolute'/>")
					.find(':last')[0];
			d.src = a.src;
			d.style.filter = "flipv progid:DXImageTransform.Microsoft.Alpha(opacity="
					+ (c * 100)
					+ ", style=1, finishOpacity=0, startx=0, starty=0, finishx=0, finishy="
					+ (b / imageHeight * 100) + ")"
		} else {
			this.element = d = parent.append(
					"<canvas class='reflection' style='position:absolute'/>")
					.find(':last')[0];
			if (!d.getContext) {
				return
			}
			...
			try {
				...
				gradient
						.addColorStop(0, "rgba(255, 255, 255, " + (1 - c) + ")");
	...
	var s = function(a, b) {
		...
		if (this.options.reflHeight > 0) {
			this.reflection = new Reflection(this.image,
					this.options.reflHeight, this.options.reflOpacity)
		}
	...
	var t = function(o, p, q) {
		...
		$(o).bind('mouseover', this, function(a) {
		...
		this.rotate = function(a) {
			...
			if (this.frontIndex < 0) {
				this.frontIndex = r.length - 1
			}
			if (this.frontIndex > r.length - 1) {
				this.frontIndex = 0
			}
		...
		this.updateAll = function() {
			...
			for ( var i = 0; i < e; i++) {
				...
				scale = ((sinVal + 1) * b) + a;
				x = this.xCentre
						+ (((funcCos(g) * this.xRadius) - (item.orgWidth * 0.5)) * scale);
		...
		this.checkImagesLoaded = function() {
			...
			for (i = 0; i < p.length; i++) {
				if ((p[i].width === undefined)
						|| ((p[i].complete !== undefined) && (!p[i].complete))) {
					return
				}
			}
			for (i = 0; i < p.length; i++) {
	...
	$.fn.CloudCarousel = function(a) {
		this.each(function() {
			...
			$(this).data('cloudcarousel',
					new t(this, $('.cloudcarousel', $(this)), a))
		});
		return this
	}
})(jQuery);