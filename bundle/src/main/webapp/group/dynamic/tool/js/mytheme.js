	...
	this.put = function(key, value) {
		...
		return $.param({
			model : $.extend({}, {
				...
				style : $.toJSON(instance.style)
			}, arg)
		}).replace(/%5B(\D+)%5D/g, ".$1")
	}, this.save = function() {
		if (layout.layoutId)
			$.get(contextRoot + "/rest/laytheme/save?" + instance.args({
		...
		(function() {
			$.ajax({
				url : contextRoot + "/rest/laytheme/saveAs?" + instance.args(),
				dataType : "text json",
				cache : false,
				error : function() {
					tempLi.remove();
		...
		if (confirm("您确定要删除吗？")) {
			$.ajax({
				url : contextRoot + "/rest/laytheme/delete?laythemeId="
