...
function focustoRemind(objid){
			...
			//当焦点手机
			if (objid=="mobile"){
				document.getElementById("mobile_info").innerHTML="方便客户及时联系到您！ABBCC公司不绑定任何收费服务。";
...
function checkNick() {
	...
	$.getJSON("userValidate?randomNumber="+Math.random(), {
		username:nick
	}, function(result) {
		if(result.affectRows==-1){
			document.getElementById("loginid_info").innerHTML="用户名已经存在";
			info_check_clean("loginid_info_check");
			document.getElementById("loginid_info").className="promptMsg2";
			return false;
		}else{
			document.getElementById("loginid_info").innerHTML="可以注册";
			info_check("loginid_info_check");
			document.getElementById("loginid_info").className="promptMsg3";
		}
	});
	return true;
}
function checkNickEmail(){
	...
	$.getJSON("emailValidate?randomNumber="+Math.random(), {
		email:nick
	}, function(result) {
		if(result.affectRows==-1){
			document.getElementById("email_info").innerHTML = "邮箱已经存在";
			info_check_clean("email_info_check");
			document.getElementById("email_info").className="promptMsg2";
		
			return false;
		}else{
			document.getElementById("email_info").innerHTML="可以使用";
			info_check("email_info_check");
			document.getElementById("email_info").className="promptMsg3";
		}
	});
