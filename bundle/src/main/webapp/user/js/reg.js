				...
				blur : function() {
					if ($(this).val() == '') {
						...
						if (validateEmail($(this).val()) == 1) {
							...
							$.getJSON("/user/emailValidate?randomNumber="
									+ Math.random(), {
								email : $("#email").val()
							}, function(result) {
								if (result.affectRows == -1) {
									siteState("email", "email已经存在", "onError");
								} else {
									siteState("email", "填写正确", "onSuccess");
								}
							});
						}
					}
	...
	$("#username").bind(
			{
				...
				blur : function() {
					if ($(this).val() == '') {
						...
						$.getJSON("/user/userValidate?randomNumber="
								+ Math.random(), {
							username : $("#username").val()
						}, function(result) {
							if (result.affectRows == -1) {
								siteState("username", "用户名已经存在", "onError");
							} else {
								siteState("username", "填写正确", "onSuccess");
							}
						});
