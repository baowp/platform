...
OnLoad.prototype = {
	...
	topic : function() {
		...
		$(":checkbox.maintain").change(
				function() {
					if (this.checked)
						mytheme.setup(this);
					else {
						var meta = $(this).metadata();
						mytheme.setup(this, $.parseJSON('{"' + meta.property
								+ '":"' + meta.normal + '"}'));
					}
				});
	...
	layout : function() {
		$("#layoutEditor .aCheck").removeClass("aCheck");
		if ($(".sideLeft").length) {
			$("#layoutEditor .sl").addClass("aCheck");
			layout.bodyType = bodyType.sl;
		} else if ($(".sideRight").length) {
			$("#layoutEditor .sr").addClass("aCheck");
			layout.bodyType = bodyType.sr;
		}
	},
	flash : function() {
	...
	advance : function() {
		if (setting.footer == "none")
			Id("footernone").checked = true;
		else
			Id("footerdisplay").checked = true;
		$("input[name=footer]").change(function() {
			setting.footer = this.value;
		});
		
		var id = "pshow" + setting.pshow;
		Id(id).checked = true;
		$("input[name=pshow]").change(function() {
			setting.pshow = parseInt(this.value);
		});
	},
	onunload : function() {
