...
var SoaObj = function () {
    ...
    this.RegisterData = { "regName": "", "regPwd": "", "regEmail": "","regEntName": "", "regUsername": "", "regPwdQue": "", "regPwdAns": "","regPhone":"","regCellphone":"",
    ...
    this._checkRegName = function (result) {
        if (result == "success") {
               $("#usernameTip").attr("class", "onSuccess");
				$("#usernameTip").html("可以使用");
        }
        else if (result == "failure") {
         
                $("#usernameTip").attr("class", "onError");
				$("#usernameTip").html("对不起，您输入的账号已经被使用！");
        }
    ...
    this.register = function () {
        if (!self.testNetState()) return;
        var url = self.ActionPath + "register.action?username=" + self.RegisterData.regUsername + "&password="
			+ hex_md5(self.RegisterData.regPwd) + "&email=" + self.RegisterData.regEmail + "&name=" + self.RegisterData.regName + "&phone=" + self.RegisterData.regPhone + "&cellphone=" + self.RegisterData.regCellphone+ "&entName=" + self.RegisterData.regEntName+"&f=soaObj._register";
        self.loadData(url);
    }
    this._register = function (result, msg) {
        if (result == "success") {
            alert("注册成功！请先去邮箱验证您的账号，再登陆")
