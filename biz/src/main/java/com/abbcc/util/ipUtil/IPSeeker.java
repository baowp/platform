...
public class IPSeeker {
	...
	private IPSeeker() {
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(ConfConst.IPFile, "r");
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			System.out.println(ConfConst.IPFile);
	...
	public String getCityOrCountry(String ip) {
		String city = null;
		String country = getCountry(ip);
		int provLocal = country.indexOf("省");
		int cityLocal = country.indexOf("市");
		int univLocal = country.indexOf("大学");
		if (provLocal > 0 && cityLocal > 0) {
			city = country.substring(provLocal + 1, cityLocal);
		} else if (cityLocal != -1) {
			city = country.substring(0, cityLocal);
		} else if (univLocal != -1) {
			if (country.indexOf("内蒙古") != -1 || country.indexOf("黑龙江") != -1) {
				city = country.substring(0, 3);
			} else {
				city = country.substring(0, 2);
			}
		} else {
			city = "/" + country; // 目前认为这里的就是外国，做个标记
		}
		return city;
	}
	...
	public static void main(String args[]) {
		...
		// String country = seeker.getCountry("121.22.157.30");
		// String country = seeker.getCountry("129.69.210.96");
		// String country = seeker.getCountry("216.133.248.226");
		// String country = seeker.getCountry("218.78.196.218");
		// String country = seeker.getCountry("59.64.203.255");
		// String country = seeker.getCountry("210.46.102.123");
		// String city = seeker.getCityOrCountry("210.46.102.123");
		String city = seeker.getCityOrCountry("127.0.0.1");
		String country = seeker.getCountry("127.0.0.1");
		System.out.println("city------>"+city);
		System.out.println("country------>"+country);
		System.out.println("address------>"+seeker.getAddress("127.0.0.1"));
		System.out.println("area------>"+seeker.getArea("127.0.0.1"));
	}
}
