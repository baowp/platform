...
public class ModuleDAOImpl extends BaseDAOImpl implements ModuleDAO {
	...
	private static final Log log = LogFactory.getLog(ModuleDAOImpl.class);
	...
	public AbcModule findById(String id) {
		...
		try {
			AbcModule instance = (AbcModule)super.findById(AbcModule.class, id);
			return instance;
	...
	public List<AbcModule> findByExample(AbcModule instance) {
		...
		try {
			List<AbcModule> results = (List<AbcModule>) super.findByExample(instance);
	...
	public List<AbcModule> findAll() {
		...
		try {
			return (List<AbcModule>)super.findAll(AbcModule.class);
		} catch (RuntimeException re) {
