...
public class LayoutDAOImpl extends BaseDAOImpl implements LayoutDAO {
	...
	private static final Log log = LogFactory.getLog(LayoutDAOImpl.class);
	...
	public AbcLayout findById(String id) {
		...
		try {
			AbcLayout instance = (AbcLayout) super
					.findById(AbcLayout.class, id);
			return instance;
	...
	public List<AbcLayout> findByExample(AbcLayout instance) {
		...
		try {
			List<AbcLayout> results = (List<AbcLayout>) super
	...
	public List<AbcLayout> findAll() {
		...
		try {
			return (List<AbcLayout>) super.findAll(AbcLayout.class);
		} catch (RuntimeException re) {
	...
	public AbcLayout merge(AbcLayout entity) {
		return (AbcLayout) getHibernateTemplate().merge(entity);
	}
