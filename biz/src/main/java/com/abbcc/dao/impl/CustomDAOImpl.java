...
public class CustomDAOImpl extends BaseDAOImpl implements CustomDAO {

	private static final Log log = LogFactory.getLog(CustomDAOImpl.class);
	...
	public AbcCustom findById(String id) {
		...
		try {
			AbcCustom instance = (AbcCustom) super.findById(
					AbcCustom.class, id);
			return instance;
	...
	public List<AbcCustom> findByExample(AbcCustom instance) {
		...
		try {
			List<AbcCustom> results = (List<AbcCustom>) super
	...
	public List<AbcCustom> findAll() {
		...
		try {
			return (List<AbcCustom>) super.findAll(AbcCustom.class);
		} catch (RuntimeException re) {
	...
	public int updateColumn(Serializable id, String column, String value) {
		String hql = "update AbcCustom set " + column + "='" + value
				+ "' where categoryId='" + id + "'";
