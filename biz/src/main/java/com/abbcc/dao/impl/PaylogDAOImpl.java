...
public class PaylogDAOImpl extends BaseDAOImpl implements PaylogDAO {
	...
	public AbcPaylog findById(String id) {
		...
		try {
			AbcPaylog instance = (AbcPaylog)super.findById(AbcPaylog.class, id);
			return instance;
	...
	public List<AbcPaylog> findByExample(AbcPaylog instance) {
		...
		try {
			List<AbcPaylog> results = (List<AbcPaylog>) super.findByExample(instance);
	...
	public List<AbcPaylog> findAll() {
		...
		try {
			return (List<AbcPaylog>)super.findAll(AbcPaylog.class);
		} catch (RuntimeException re) {
